...
namespace Networks.Example.XOr
{
    public class Network
    {
		...
		public void Initialize()
		{
			...
			Input1 = new Node(ActivationMethodType.Linear);
			Input2 = new Node(ActivationMethodType.Linear);
			Output = new Node(ActivationMethodType.Linear);

			for (int i = 0; i < NumHiddenLayerNodes; i++)
			{
				HiddenLayer.Add(new Node(ActivationMethodType.Linear));
			}
